<?php
session_start();
include '../../config/database.php';

if (!isset($_SESSION['username'])) {
    header("Location: ../../modules/auth/login.php");
    exit();
}

// Ambil daftar user dari tbl_user
$users_query = "SELECT id, nama FROM tbl_user ORDER BY nama";
$users_result = mysqli_query($conn, $users_query);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id = intval($_POST['id']);
    $jabatan = mysqli_real_escape_string($conn, $_POST['jabatan']);
    
    // Validasi input
    if (empty($id) || empty($jabatan)) {
        header("Location: create.php?error=Semua field harus diisi");
        exit();
    }

    // Cek apakah user sudah memiliki jabatan
    $check_query = "SELECT id FROM tbl_jabatan WHERE id = $id";
    $check_result = mysqli_query($conn, $check_query);
    
    if (mysqli_num_rows($check_result) > 0) {
        header("Location: create.php?error=User ini sudah memiliki jabatan");
        exit();
    }

    // Ambil nama pegawai berdasarkan ID yang dipilih
    $nama_query = "SELECT nama FROM tbl_user WHERE id = $id";
    $nama_result = mysqli_query($conn, $nama_query);
    
    if (mysqli_num_rows($nama_result) == 0) {
        header("Location: create.php?error=User tidak ditemukan");
        exit();
    }
    
    $user_data = mysqli_fetch_assoc($nama_result);
    $nama = mysqli_real_escape_string($conn, $user_data['nama']);

    // Insert data ke database
    $query = "INSERT INTO tbl_jabatan (id, nama, jabatan) VALUES ('$id', '$nama', '$jabatan')";
    
    if (mysqli_query($conn, $query)) {
        header("Location: index.php?success=Data jabatan berhasil ditambahkan");
        exit();
    } else {
        header("Location: create.php?error=Gagal menambahkan data jabatan: " . mysqli_error($conn));
        exit();
    }
}
?>

<?php include '../../includes/header.php'; ?>

<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h4 class="mb-0">Tambah Jabatan Baru</h4>
                </div>
                <div class="card-body">
                    <?php if (isset($_GET['error'])): ?>
                        <div class="alert alert-danger"><?php echo htmlspecialchars($_GET['error']); ?></div>
                    <?php endif; ?>
                    
                    <form action="create.php" method="POST">
                        <div class="mb-3">
                            <label for="id" class="form-label">Nama Pegawai</label>
                            <select class="form-select" id="id" name="id" required>
                                <option value="">-- Pilih Pegawai --</option>
                                <?php while ($user = mysqli_fetch_assoc($users_result)): ?>
                                    <option value="<?php echo $user['id']; ?>">
                                        <?php echo htmlspecialchars($user['nama']); ?>
                                    </option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="jabatan" class="form-label">Jabatan</label>
                            <input type="text" class="form-control" id="jabatan" name="jabatan" required>
                        </div>
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-save"></i> Simpan
                            </button>
                            <a href="index.php" class="btn btn-secondary">
                                <i class="bi bi-arrow-left"></i> Kembali
                            </a>
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>

<?php include '../../includes/footer.php'; ?>